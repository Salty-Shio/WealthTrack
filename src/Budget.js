...
class Budget {
  ...
  addTransaction(categoryId, date, name, transfer, amount) {
    ...
    const newId =
      category.transactions.length > 0
        ? category.transactions[category.transactions.length - 1].id + 1
        : 1;
    const transaction = {
      id: newId,
      date,
  ...
  getTransaction(categoryId, transactionId) {
    ...
    return category.transactions.find(
      (transaction) => transaction.id === transactionId
    );
  }
  ...
  calculateCategoryRemainingBalance(categoryId) {
    ...
    console.log(
      `Number of category transactions: ${category.transactions.length}`
    );
    ...
    console.log(
      `Category ${category.envelope} remaining balance: ${remainingBalance}`
    );
  ...
  calculateOverallRemainingBalance() {
    ...
    for (const category of this.categories) {
      const categoryExpendedBalance =
        category.totalBalance - category.remainingBalance;
